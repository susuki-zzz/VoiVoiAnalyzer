...
std::vector<double> computeExpectedCepstrum(const std::vector<Sample>& samples,
    ...
    for (size_t k = 0; k < fft_size; ++k) {
        log_spectrum[k] = std::log(std::abs(spectrum[k]) + 1e-10);
    }
    ...
    for (size_t n = 0; n < fft_size; ++n) {
        ...
        for (size_t k = 0; k < fft_size; ++k) {
            const double angle = 2.0 * std::numbers::pi * static_cast<double>(k * n) /
                                  static_cast<double>(fft_size);
            sum += std::complex<double>(log_spectrum[k], 0.0) *
...
int runCPPAnalyzerCepstrumTest() {
    AudioConfig config;
    ...
    if (!approximatelyEqual(static_cast<double>(cpp), expected_cpp, 1e-3)) {
        return 2;
    }
...
int main() {
    if (int result = runCPPAnalyzerCepstrumTest(); result != 0) {
        return result;
    }
