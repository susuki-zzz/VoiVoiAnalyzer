...
void AudioInputManager::closeDevice() {
    if (device_) { stop(); device_.reset(); LOG_INFO("Audio device closed"); }
    deviceName_.clear(); actualSampleRate_ = 0; actualBufferSize_ = 0; actualChannels_ = 0; ringBuffer_->clear(); resetStatistics();
}
...
void AudioInputManager::start() {
    if (!device_) { LOG_ERROR("Cannot start: no device open"); return; }
    if (device_->isRunning()) { LOG_WARN("Device already running"); return; }
    ringBuffer_->clear(); resetStatistics(); device_->start(); LOG_INFO("Audio input started");
}
...
void AudioInputManager::stop() {
    if (!device_) return; if (!device_->isRunning()) return; device_->stop(); LOG_INFO("Audio input stopped");
}
...
void AudioInputManager::audioCallback(const float* samples, size_t frames, double sampleRate) {
    ...
    if (written < frames) {
        ...
        if (onXRun) { if (auto* mm = juce::MessageManager::getInstance()) mm->callAsync([this]{ if(onXRun) onXRun(); }); }
...
void AudioInputManager::updateStatistics(const float* samples, size_t frames) {
    std::lock_guard<std::mutex> lock(statsMutex_); stats_.totalSamplesReceived += frames; stats_.totalCallbacks++;
    float peak = 0.0f; double sumSquares = 0.0; for (size_t i = 0; i < frames; ++i) { float absVal = std::abs(samples[i]); peak = std::max(peak, absVal); sumSquares += static_cast<double>(samples[i]) * static_cast<double>(samples[i]); }
    stats_.peakLevel = std::max(stats_.peakLevel, static_cast<double>(peak));
    if (frames > 0) { double rms = std::sqrt(sumSquares / static_cast<double>(frames)); const double alpha = 0.1; if (stats_.totalCallbacks == 1) { stats_.rmsLevel = rms; } else { stats_.rmsLevel = alpha * rms + (1.0 - alpha) * stats_.rmsLevel; } }
}
