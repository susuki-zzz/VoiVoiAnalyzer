...
namespace yvc::app {
    ...
    /// </summary>
    void WaveformComponent::setSamples(const std::vector<float>& samples,
                                       float sampleRate) {
        samples_ = samples;
        ...
        minY_ = mn;
        maxY_ = mx;
        if(std::fabs(maxY_ - minY_) < 1e-6f) maxY_ = minY_ + 1.0f;
        repaint();
    }
    ...
    void WaveformComponent::paint(juce::Graphics& g) {
        ...
        float w = b.getWidth();
        float h = b.getHeight();
        float range = (maxY_ - minY_) + 1e-12f;
        for(size_t i = 0; i < N; ++i) {
            float x = b.getX() + (float)i * w / (float)(N - 1);
            float norm = (samples_[i] - minY_) / range;
