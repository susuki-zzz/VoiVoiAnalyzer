...
namespace { // anonymous
    ...
    public:
        ...
        void suspended() override {
            if(audioDeviceManager_) audioDeviceManager_->closeAudioDevice();
        }
        void resumed() override {
            if(audioDeviceManager_) audioDeviceManager_->restartLastAudioDevice();
        }
        ...
        public:
            ...
            void moved() override {
                if(auto* s = appProperties_.getUserSettings()) {
                    s->setValue("windowX", getX());
                    s->setValue("windowY", getY());
                    s->saveIfNeeded();
                }
            }
            void resized() override {
                ...
                if(auto* s = appProperties_.getUserSettings()) {
                    s->setValue("windowWidth", getWidth());
                    s->setValue("windowHeight", getHeight());
                    s->saveIfNeeded();
                }
            }
        private:
