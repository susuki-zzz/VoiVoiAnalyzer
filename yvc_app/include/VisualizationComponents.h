...
namespace yvc::app {
    ...
    protected:
        ...
        double timestampToX(double ts, juce::Rectangle<int> area) const;
        double xToTimestamp(int x, juce::Rectangle<int> area) const;
        friend class test::AdvancedHeatmapComponentTestPeer;
    };

    inline AdvancedHeatmapComponent::AdvancedHeatmapComponent(const juce::String& t) :
        title_(t)
    {
        setConfig(HeatmapConfig{});
    }
    inline void AdvancedHeatmapComponent::setConfig(const HeatmapConfig& c) {
        config_ = c;
        repaint();
    }
    inline void AdvancedHeatmapComponent::appendSample(float v, double ts) {
        samples_.push_back(Sample{ v,ts,std::isfinite(v) });
        while((int)samples_.size() > config_.maxSamples) samples_.pop_front();
        if(!samples_.empty()) {
            double latest = samples_.back().timestamp;
            if(latest > zoomEnd_) {
                double span = zoomEnd_ - zoomStart_;
                if(span <= 0)
                {
                    span = 1.0;
    ...
    inline juce::Image AdvancedHeatmapComponent::createExportImage(int w, int h, bool ann) {
        juce::Image img(juce::Image::RGB, w, h, true);
        juce::Graphics g(img);
        g.fillAll(juce::Colours::black);
        g.setColour(juce::Colours::white);
        if(ann) g.drawText(title_, 0, 0, w, 24, juce::Justification::centred);
        drawHeatmapData(g, juce::Rectangle<int>(0, ann ? 24 : 0, w, h - (ann ? 24 : 0)));
        return img;
    }
    inline bool AdvancedHeatmapComponent::exportToPNG(const juce::File& f, int w, int h) {
        auto img = createExportImage(w, h, true);
        juce::PNGImageFormat fmt;
        juce::FileOutputStream out(f);
        return out.openedOk() && fmt.writeImageToStream(img, out);
    }
    inline void AdvancedHeatmapComponent::paint(juce::Graphics& g) {
        auto area = getLocalBounds();
        g.fillAll(juce::Colours::darkgrey.darker());
        g.setColour(juce::Colours::white);
        g.drawText(title_, area.removeFromTop(24), juce::Justification::centred);
        heatmapArea_ = area;
        drawHeatmapData(g, heatmapArea_);
        drawPlayhead(g, heatmapArea_);
        if(isZooming_) drawZoomOverlay(g);
    }
    inline void AdvancedHeatmapComponent::resized() { repaint(); }
    inline void AdvancedHeatmapComponent::mouseDown(const juce::MouseEvent& e) {
        if(e.mods.isShiftDown()) {
            isZooming_ = true;
            zoomStartPoint_ = e.getPosition();
        }
        else {
            double ts = xToTimestamp(e.x, heatmapArea_);
            if(onScrubPositionChanged) onScrubPositionChanged(ts);
        }
    }
    inline void AdvancedHeatmapComponent::mouseDrag(const juce::MouseEvent& e) {
    ...
    inline void AdvancedHeatmapComponent::mouseWheelMove(const juce::MouseEvent& e, const juce::MouseWheelDetails& wheel) {
        if(!heatmapArea_.contains(e.getPosition())) return;
        double span = zoomEnd_ - zoomStart_;
        double factor = 1.0 - wheel.deltaY * 0.2;
        span *= factor;
        double center = (zoomStart_ + zoomEnd_) / 2.0;
        zoomStart_ = center - span / 2.0;
        zoomEnd_ = center + span / 2.0;
        if(onZoomRangeChanged) onZoomRangeChanged(zoomStart_, zoomEnd_);
        repaint();
    }
    inline void AdvancedHeatmapComponent::drawHeatmapData(juce::Graphics& g, juce::Rectangle<int> area) {
        if(samples_.empty()) {
        ...
        g.fillRect(area);
        double minT = zoomStart_, maxT = zoomEnd_;
        if(maxT <= minT) maxT = minT + 1.0;
        for(auto& s : samples_) {
            if(s.timestamp<minT || s.timestamp>maxT) continue;
            float normT = float((s.timestamp - minT) / (maxT - minT));
            float normV = juce::jlimit(0.0f, 1.0f, (s.value - config_.minValue) / (config_.maxValue - config_.minValue));
            int x = area.getX() + int(normT * area.getWidth());
            int y = area.getBottom() - int(normV * area.getHeight());
            g.setColour(getValueColour(s.value));
            g.fillRect(x, y, 2, 2);
        }
    }
    ...
    inline void AdvancedHeatmapComponent::drawPlayhead(juce::Graphics& g, juce::Rectangle<int> area) {
        if(playheadPosition_ >= zoomStart_ && playheadPosition_ <= zoomEnd_) {
            float norm = float((playheadPosition_ - zoomStart_) / (zoomEnd_ - zoomStart_));
            int x = area.getX() + int(norm * area.getWidth());
            g.setColour(juce::Colours::yellow);
            g.drawLine((float)x, (float)area.getY(), (float)x, (float)area.getBottom(), 1.5f);
        }
    }
    inline void AdvancedHeatmapComponent::drawZoomOverlay(juce::Graphics& g) {
        g.setColour(juce::Colours::white.withAlpha(0.15f));
        g.fillRect(heatmapArea_);
    }
    inline juce::Colour AdvancedHeatmapComponent::getValueColour(float v) const {
        float nv = juce::jlimit(0.0f, 1.0f, (v - config_.minValue) / (config_.maxValue - config_.minValue));
        return config_.lowColour.interpolatedWith(config_.highColour, nv);
    }
    inline double AdvancedHeatmapComponent::timestampToX(double ts, juce::Rectangle<int> area) const {
        if(zoomEnd_ <= zoomStart_) return area.getX();
        return area.getX() + (ts - zoomStart_) * (area.getWidth() / (zoomEnd_ - zoomStart_));
    }
    inline double AdvancedHeatmapComponent::xToTimestamp(int x, juce::Rectangle<int> area) const {
        return zoomStart_ + (double(x - area.getX()) / double(area.getWidth())) * (zoomEnd_ - zoomStart_);
    }
    ...
    public:
        ...
        void addSession(const juce::String& name,
                        const std::vector<yvc::AnalysisResults>& data,
    ...
    protected:
        ...
        float getMetricValue(const yvc::AnalysisResults& r, const juce::String& metric);
    ...
    inline void ComparativeMetricsComponent::addSession(const juce::String& n, const std::vector<yvc::AnalysisResults>& d, juce::Colour c) {
        sessions_.push_back(SessionData{ n,c,d,1.0f,true });
        repaint();
    }
    inline void ComparativeMetricsComponent::removeSession(const juce::String& n) {
    ...
    inline void ComparativeMetricsComponent::setSessionVisibility(const juce::String& n, bool v) {
        for(auto& s : sessions_)
        {
            if(s.name == n) s.visible = v;
        }
        repaint();
    }
    inline void ComparativeMetricsComponent::setSessionAlpha(const juce::String& n, float a) {
        for(auto& s : sessions_)
            if(s.name == n) s.alpha = juce::jlimit(0.0f, 1.0f, a);
        repaint();
    }
    ...
    inline void ComparativeMetricsComponent::paint(juce::Graphics& g) {
        auto area = getLocalBounds();
        g.fillAll(juce::Colours::black);
        g.setColour(juce::Colours::white);
        g.drawText("Comparative", area.removeFromTop(20), juce::Justification::centred);
        for(auto& ses : sessions_) if(ses.visible) drawSession(g, ses, area);
    }
    inline void ComparativeMetricsComponent::resized() { repaint(); }
    inline void ComparativeMetricsComponent::drawSession(juce::Graphics& g, const SessionData& s, juce::Rectangle<int> area) {
        if(s.data.empty()) return;
        auto range = getMetricRange(currentMetric_);
        float minV = range.first, maxV = range.second;
        g.setColour(s.colour.withAlpha(s.alpha));
        juce::Path p;
        bool started = false;
        for(auto& r : s.data) {
            if(r.timestamp<timeStart_ || r.timestamp>timeEnd_) continue;
            float tNorm = float((r.timestamp - timeStart_) / (timeEnd_ - timeStart_ + 1e-9));
            float v = getMetricValue(r, currentMetric_);
            float vNorm = (v - minV) / (maxV - minV + 1e-9f);
            float x = area.getX() + tNorm * area.getWidth();
            float y = area.getBottom() - vNorm * area.getHeight();
            if(!started) {
    ...
    inline float ComparativeMetricsComponent::getMetricValue(const yvc::AnalysisResults& r, const juce::String& m) {
        if(m == "f0") return r.f0;
        if(m == "rms") return r.rms;
        if(m == "cpp") return r.cpp;
        if(m == "hnr") return r.hnr;
        if(m == "speech_rate") return r.speech_rate;
        if(m == "pause_ratio") return r.pause_ratio;
        if(m == "spectral_tilt") return r.spectral_tilt;
        return r.rms;
    }
    inline juce::String ComparativeMetricsComponent::getMetricUnit(const juce::String& m) {
        if(m == "f0") return "Hz";
        if(m == "rms" || m == "cpp" || m == "hnr") return "dB";
        if(m == "speech_rate") return "syll/s";
        if(m == "pause_ratio") return "%";
        if(m == "spectral_tilt") return "dB/oct";
        return "";
    }
    inline std::pair<float, float> ComparativeMetricsComponent::getMetricRange(const juce::String& m) {
        if(m == "f0") return { 50.0f,500.0f };
        if(m == "rms") return { 0.0f,1.0f };
        if(m == "cpp") return { -10.0f,30.0f };
        if(m == "hnr") return { 0.0f,40.0f };
        if(m == "speech_rate") return { 0.0f,8.0f };
        if(m == "pause_ratio") return { 0.0f,1.0f };
        if(m == "spectral_tilt") return { -24.0f,12.0f };
        return { 0.0f,100.0f };
    }
    ...
    public:
        ...
        bool isF0Valid() const { return f0Valid_; }
        bool isHarmonicsOverlayEnabled() const { return harmonicsOverlayEnabled_; }
        float mapFrequencyToXPublic(float freq, juce::Rectangle<int> area) const { return frequencyToX(freq, area); }
        int mapFrequencyToBinPublic(float freq) const { return frequencyToBin(freq); }
        float mapMagnitudeToYPublic(float m, juce::Rectangle<int> area) const { return magnitudeToY(m, area); }
        void paint(juce::Graphics& g) override;
        void resized() override;
    ...
    protected:
        ...
        float magnitudeToY(float magnitude, juce::Rectangle<int> area) const;
