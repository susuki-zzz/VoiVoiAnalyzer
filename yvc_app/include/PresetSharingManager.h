...
public:
    ...
    struct MetricTarget { std::string metricName; float minValue = 0.0f; float maxValue = 100.0f; float targetValue = 50.0f; bool showBaseline = false; std::string unit; };
    /// <summary>Layout configuration for one UI component within a preset.</summary>
    struct LayoutConfig { std::string componentType; int gridColumn = 0; int gridRow = 0; int spanColumns = 1; int spanRows = 1; bool visible = true; };
    /// <summary>Shareable preset container (metrics only).</summary>
    struct ShareablePreset {
        std::string name; std::string description; std::string category; std::string language; std::string createdBy; std::string version = "1.0"; int64_t timestamp = 0; std::vector<MetricTarget> metricTargets; std::vector<LayoutConfig> layoutConfig; std::string checksum; bool isValidated = false; };
...
private:
    ...
    juce::File getPresetsDirectory() const; std::string generatePresetId(const ShareablePreset& preset) const; bool isValidPresetName(const std::string& name) const;
    mutable std::mutex mutex_; std::vector<ShareablePreset> localPresets_;
};
...
public:
    ...
    // TableListBoxModel
    int getNumRows() override; void paintRowBackground(juce::Graphics& g, int rowNumber, int width, int height, bool rowIsSelected) override; void paintCell(juce::Graphics& g, int rowNumber, int columnId, int width, int height, bool rowIsSelected) override; void cellClicked(int rowNumber, int columnId, const juce::MouseEvent& e) override; juce::Component* refreshComponentForCell(int rowNumber, int columnId, bool isRowSelected, juce::Component* existingComponentToUpdate) override;
    // Button::Listener
    void buttonClicked(juce::Button* button) override;
    // FilenameComponentListener
    void filenameComponentChanged(juce::FilenameComponent*) override;
    // Callbacks
    std::function<void(const PresetSharingManager::ShareablePreset&)> onPresetSelected; std::function<void(const std::string&)> onPresetDeleted;
private:
    void refreshPresetList(); void updateUI(); juce::TableListBox presetTable_; juce::TextButton importButton_{ "Import Preset" }; juce::TextButton exportButton_{ "Export Selected" }; juce::TextButton deleteButton_{ "Delete Selected" }; juce::FilenameComponent fileChooser_{ "Choose preset file", {}, true, false, false, "*.json", {}, "Select preset file to import" }; std::vector<PresetSharingManager::ShareablePreset> displayedPresets_; int selectedRow_ = -1;
};
